...
def common_shape(x, y):
    ...
    if x is None:
        # The associated input was not a Tensor, no shape generated.
        return None
...
def is_same_structure(structure1,
    ...
    try:
        nest.assert_same_structure(
            structure1, structure2, expand_composites=True)
...
class _EagerDefinedFunction(object):
    ...
    def __init__(self, name, graph, inputs, outputs, attrs):
        ...
        operations = [op for op in graph.get_operations()
        ...
        fn = pywrap_tf_session.TF_GraphToFunction_wrapper(
            ...
            [t._as_tf_output()
             for t in inputs],  # pylint: disable=protected-access
            [t._as_tf_output()
             for t in outputs],  # pylint: disable=protected-access
            output_names,
        ...
        with ops.init_scope():
            if context.executing_eagerly():
                ...
                self._function_deleter = _EagerDefinedFunctionDeleter(
                    self.name)
        ...
        self._stateful_ops = tuple(
            op for op in operations if op._is_stateful)  # pylint: disable=protected-access
...
class _DelayedRewriteGradientFunctions(object):
    ...
    def _rewrite_forward_and_call_backward(self, op, *doutputs):
        ...
        forward_function, backwards_function = self.forward_backward(
            len(doutputs))
        ...
        for doutput, placeholder in zip(doutputs, self._func_graph.outputs):
            if backprop_util.IsTrainable(placeholder):
                if isinstance(doutput, indexed_slices.IndexedSlices):
                    ...
                    cleaned_doutputs.append(
                        default_gradient.zeros_like(placeholder))
...
class _TapeGradientFunctions(object):
        ...
        with backwards_graph.as_default():
            ...
            for output in trainable_outputs:
                gradient_shape, gradient_dtype = default_gradient.shape_and_dtype(
                    output)
                gradient_placeholder = graph_placeholder(
            ...
            if input_tangents:
                ...
                gradients_wrt_inputs = nest.map_structure(
                    lambda x: ops.convert_to_tensor(
            ...
            with forwardprop_util.push_forwardprop_state():
                ...
                for input_index, real_input in enumerate(self._func_graph.inputs):
                    ...
                    if capture is not None:
                        forward_wrapper_graph.add_capture(
                ...
                for tape_indices in self._forwardprop_input_indices:
                    for input_index, jvp_index in tape_indices:
                        ...
                        jvp_placeholder = graph_placeholder(
                            gradient_dtype, gradient_shape)
                        external_jvp = input_tangents[jvp_index -
                                                      num_inference_inputs]
                        forward_wrapper_graph.add_capture(
                            external_jvp, jvp_placeholder)
    ...
    def _wrap_backward_function(self, forward_graph, backward, outputs):
        ...
        backward_function_inputs = (
            len(backward.inputs) - len(captured_inputs))
        recorded_outputs = []
        ...
        for output_index, output in enumerate(relevant_outputs):
            ...
            if output.dtype == dtypes.variant:
                variant_zeros_like[output_index] = default_gradient.zeros_like(
                    output)
...
class ConcreteFunction(core.ConcreteFunction, trackable.Trackable):
    ...
    def _initialize_function_spec(self):
        ...
        if self._pre_initialized_function_spec is None:
            # e.g., SavedBareConcreteFunction doesn't have function_spec yet.
            return
        ...
        fullargspec = tf_inspect.FullArgSpec(
            args=list(args) + ["<arg{}>".format(i + 1)
                               for i in vararg_indices],
            varargs=None,
            varkw=None,
    ...
    def _structured_signature_check_arg_types(self, args, kwargs):
        ...
        for (name, arg) in kwargs.items():
            self._structured_signature_check_arg_type(
                arg, kwarg_specs[name], name)
    ...
    def _structured_signature_check_arg_type(self, arg, spec, name):
        ...
        try:
            ...
            try:
                ...
                expected, got = _structure_summary(
                    spec), _structure_summary(arg)
        ...
        for (arg_piece, spec_piece) in zip(arg_pieces, spec_pieces):
            ...
            if isinstance(spec_piece, tensor_spec.DenseSpec):
                ...
                try:
                    ...
                    logging.vlog(
                        1, "Error matching value with spec", exc_info=True)
    ...
    def replace_capture_with_deferred_capture(self,
        ...
        if placeholder is None:
            ...
            placeholder = self.inputs[-len(self._captured_inputs) +
                                      capture_index]
    ...
    def _select_forward_and_backward_functions(
        ...
        if (possible_gradient_type
            ...
            if input_tangents.indices or executing_eagerly:
                ...
                functions = self._first_order_tape_functions.get(
                    cache_key, None)
...
class FunctionSpec(object):
        ...
        if isinstance(unwrapped, functools.partial):
            ...
            if fullargspec.defaults or fullargspec.kwonlydefaults:
                ...
                if fullargspec.defaults:
                    ...
                    no_default = object()
                    num_args_without_defaults = len(
                        old_args) - len(old_defaults)
                    left_padding = tuple(
                        [no_default] * num_args_without_defaults)

                    args_with_defaults = zip(
                        old_args, left_padding + old_defaults)
        ...
        if self._fullargspec.kwonlyargs:
            ...
            for arg_name in self._fullargspec.kwonlyargs:
                ...
                if default_values and arg_name in self._fullargspec.kwonlydefaults:
                    args[-1] += "={}".format(
                        self._fullargspec.kwonlydefaults[arg_name])
    ...
    def _convert_annotated_args_to_tensors(self, args, kwargs):
        ...
        for kw, v in kwargs.items():
            ...
            kwarg_annotation = self._fullargspec.annotations.get(
                annotation_key, None)
    ...
    def canonicalize_function_inputs(self, *args, **kwargs):
        ...
        if self._experimental_follow_type_hints:
            args, kwargs = self._convert_annotated_args_to_tensors(
                args, kwargs)
        ...
        if not kwargs:
            ...
            if missing_arg_indices:
                missing_args = [self._arg_names[i]
                                for i in sorted(missing_arg_indices)]
        ...
        if self._input_signature is None:
            inputs, flat_inputs, filtered_flat_inputs = _convert_numpy_inputs(
                inputs)
            kwargs, flat_kwargs, filtered_flat_kwargs = _convert_numpy_inputs(
                kwargs)
        ...
        with self._lock:
            ...
            for arg in graph_function.graph.inputs:
                ...
                graph_function._arg_keywords.append(
                    proposal)  # pylint: disable=protected-access
        ...
        with monitoring.MonitoredTimer(_graph_building_time_counter.get_cell()):
            with trace.Trace("tf.function-graph_building"):
                ...
                with ag_ctx.ControlStatusCtx(
                    ...
                    self._function_cache.add_call_context(
