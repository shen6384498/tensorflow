        ...
        try:
            ...
            for index, v in enumerate(variables):
                try:
                    ...
                    components = parallel_device.unpack(
                        var_is_initialized[index])
                    with ops.device(None):
                        components = array_ops.stack(components)
                        all_initialized = math_ops.reduce_all(
                            components).numpy()
                        any_initialized = math_ops.reduce_any(
                            components).numpy()
...
class Function(core.GenericFunction, trackable.Trackable):
    ...
    def __init__(self,
        ...
        ops._tf_function_api_guage.get_cell().set(
    ...
    def _initialize(self, args, kwds, add_initializers_to=None):
        ...
        if self._input_signature is not None:
            ...
            default_arg_len = len(
                self.function_spec.fullargspec.defaults or ())
    ...
    def __call__(self, *args, **kwds):
        ...
        if self._created_variables is None:
            ...
            if ops.executing_eagerly_outside_functions() and (
                ...
                _tf_function_counter.get_cell(
                    str(int(compiled))).increase_by(1)
    ...
    def _list_all_concrete_functions_for_serialization(self):
        ...
        for concrete_function in concrete_functions:
            ...
            if any(
                ...
                logging.info(
                    "Unsupported signature for serialization: %s.", signature)
                continue
        ...
        for args, kwargs in seen_signatures:
            concrete_functions.append(
                self.get_concrete_function(*args, **kwargs))
        return concrete_functions
    ...
    def _get_concrete_function_garbage_collected(self, *args, **kwargs):
        ...
        with self._lock:
            if self._stateful_fn is None:
                initializers = []
                self._initialize(
                    args, kwargs, add_initializers_to=initializers)
                self._initialize_uninitialized_variables(initializers)
