...
PyObject* GetRegisteredType(const string& key) {
  LOG(ERROR) << "hello boy ********************************** GetRegisteredType";
...
StringPiece GetClassName(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** GetClassName";
...
string PyObjectToString(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** PyObjectToString";
  if (o == nullptr) {
    return "<null object>";
  }
...
int IsInstanceOfRegisteredType(PyObject* obj, const char* type_name) {
  LOG(ERROR) << "hello boy ********************************** IsInstanceOfRegisteredType";
...
int IsMappingHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsMappingHelper";
...
int IsMutableMappingHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsMutableMappingHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsMappingViewHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsMappingViewHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsObjectProxy(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsObjectProxy";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsAttrsHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsAttrsHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsIndexedSlicesHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsIndexedSlicesHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsTensorHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsTensorHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsEagerTensorHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsEagerTensorHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsResourceVariableHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsResourceVariableHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsVariableHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsVariableHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsSequenceHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsSequenceHelper";
...
class DictValueIterator : public ValueIterator {
 public:
  ...
  LOG(ERROR) << "hello boy ********************************** DictValueIterator";
...
bool IsSparseTensorValueType(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsSparseTensorValueType";
...
bool IsCompositeTensorHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsCompositeTensorHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
bool IsTypeSpecHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsTypeSpecHelper";
  static auto* const check_cache = new CachedTypeCheck([](PyObject* to_check) {
...
int IsSequenceOrCompositeHelper(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsSequenceOrCompositeHelper";
...
ValueIteratorPtr GetValueIteratorForData(PyObject* nested) {
  LOG(ERROR) << "hello boy ********************************** GetValueIteratorForData";
  if (PyDict_Check(nested)) {
    return absl::make_unique<DictValueIterator>(nested);
  } else if (IsMappingHelper(nested)) {
    return absl::make_unique<MappingValueIterator>(nested);
...
ValueIteratorPtr GetValueIteratorForComposite(PyObject* nested) {
  LOG(ERROR) << "hello boy ********************************** GetValueIteratorForComposite";
...
bool FlattenHelper(
  ...
  LOG(ERROR) << "hello boy ********************************** FlattenHelper";
...
void SetDifferentKeysError(PyObject* dict1, PyObject* dict2, string* error_msg,
                           bool* is_type_error) {
  LOG(ERROR) << "hello boy ********************************** SetDifferentKeysError";
...
bool AssertSameStructureHelper(
  ...
  LOG(ERROR) << "hello boy ********************************** AssertSameStructureHelper";
  DCHECK(error_msg);
...
bool IsSequence(PyObject* o) { 
  LOG(ERROR) << "hello boy ********************************** IsSequence";
  return IsSequenceHelper(o) == 1; }
...
PyObject* MappingKeys(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** MappingKeys";
#if PY_MAJOR_VERSION >= 3
...
bool IsSequenceOrComposite(PyObject* o) {
  LOG(ERROR) << "hello boy ********************************** IsSequenceOrComposite";
  return IsSequenceOrCompositeHelper(o) == 1;
}

bool IsCompositeTensor(PyObject* o) { 
  LOG(ERROR) << "hello boy ********************************** IsCompositeTensor";
...
bool IsSequenceForData(PyObject* o) { 
  ...
  return IsSequenceForDataHelper(o) == 1; }
