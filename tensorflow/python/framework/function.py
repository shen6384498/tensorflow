...
class _DefinedFunction(object):
    ...
    def _create_definition_if_needed_impl(self):
        ...
        variable_keys.extend(
            ops.GraphKeys._VARIABLE_COLLECTIONS)  # pylint: disable=protected-access
        variable_keys.append(
            vs._VARSTORE_KEY)  # pylint: disable=protected-access
        ...
        kwargs_attr = _parse_kwargs_as_attrs(
            base_func_name, **self._extra_kwargs)
    ...
    def __call__(self, *args, **kwargs):
        ...
        for (i, x) in enumerate(args):
            ...
            if not isinstance(x, ops.Tensor):
                raise ValueError(
                    f"Expected a Tensor but got {x} with type {type(x)}.")
...
def func_graph_from_py_func(func,
    ...
    with func_graph.as_default(), ops.device(device):
        ...
        for (argname, argtype, argshape) in zip(arg_names, arg_types, arg_shapes):
            argholder = array_ops.placeholder(
                argtype, shape=argshape, name=argname)
...
def _is_guaranteed_const(tensor):
    ...
    def is_guaranteed_const(op): return op.node_def.op == "GuaranteeConst"
    constants = set([])
...
def _get_kwarg_as_str_attr(attr_name, value):
    ...
    if isinstance(value, str):
        ...
        raise ValueError(
            f"Attribute {attr_name} must be str. Got {type(value)}.")
...
def _parse_kwargs_as_attrs(func_name, **kwargs):
    ...
    separate_compiled_gradients = kwargs.pop(
        "separate_compiled_gradients", None)
    if compiled is not None:
        ...
        if "_XlaScope" in ops.get_default_graph()._attr_scope_map:
            attrs["_XlaScope"] = ops.get_default_graph(
            )._attr_scope_map["_XlaScope"]
        else:
...
def _type_list_to_str(types):
    if any(_ not in _DTYPE_TO_STR for _ in types):
        unsupported_types = [
            type_ for type_ in types if type_ not in _DTYPE_TO_STR]
