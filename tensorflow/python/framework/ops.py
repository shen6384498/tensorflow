    ...
    * a shape
    ...
    def _as_tf_output(self):
        ...
        if self._tf_output is None:
            self._tf_output = c_api_util.tf_output(
...
class _EagerTensorBase(Tensor):
    ...
    def _numpy(self):
        try:
            ...
            raise core._status_to_exception(
                e) from None  # pylint: disable=protected-access
    ...
    def shape(self):
        if self._tensor_shape is None:  # pylint: disable=access-member-before-definition
            ...
            try:
                ...
                # `EagerTensor`, in C.
                self._tensor_shape = tensor_shape.TensorShape(
                    self._shape_tuple())
            except core._NotOkStatusException as e:
                raise core._status_to_exception(e) from None
...
def _create_c_op(graph, node_def, inputs, control_inputs, op_def=None):
    ...
    """
    if op_def is None:
        op_def = graph._get_op_def(
            node_def.op)  # pylint: disable=protected-access
...
class Operation(object):
    ...
    def __init__(self,
        ...
        if isinstance(node_def, node_def_pb2.NodeDef):
            ...
            if not _VALID_OP_NAME_REGEX.match(node_def.name):
                raise ValueError(
                    "'%s' is not a valid node name" % node_def.name)
            c_op = None
        ...
        if inputs is None:
            ...
            raise TypeError(
                "inputs needs to be a list of Tensors: %s" % (inputs,))
        for a in inputs:
        ...
        for i in range(num_outputs):
            ...
            tensor = Tensor._create_with_tf_output(
                self, i, output_type, tf_output)  # pylint: disable=protected-access
            self._outputs.append(tensor)

        self._id_value = self._graph._add_op(
            self, name)  # pylint: disable=protected-access
    ...
    def _remove_all_control_inputs(self):
        ...
        pywrap_tf_session.RemoveAllControlInputs(
            self._graph._c_graph, self._c_op)  # pylint: disable=protected-access
    ...
    def __tf_tensor__(self, dtype=None, name=None):
        ...
        raise TypeError(
            "can't convert Operation '{}' to Tensor".format(self.name))
    ...
    def _clear_attr(self, attr_name):
        ...
        # pylint: disable=protected-access
        pywrap_tf_session.ClearAttr(
            self._graph._c_graph, self._c_op, attr_name)
        # pylint: enable=protected-access
    ...
    def get_attr(self, name):
        ...
        try:
            with c_api_util.tf_buffer() as buf:
                pywrap_tf_session.TF_OperationGetAttrValueProto(
                    self._c_op, name, buf)
                data = pywrap_tf_session.TF_GetBuffer(buf)
        except errors.InvalidArgumentError as e:
            # Convert to ValueError for backwards compatibility.
    ...
    def _get_attr_type(self, name):
        ...
        try:
            dtype_enum = pywrap_tf_session.TF_OperationGetAttrType(
                self._c_op, name)
...
class Graph(object):
    ...
    def _resource_creator_stack(self):
        if not hasattr(self._thread_local, "_resource_creator_stack"):
            self._thread_local._resource_creator_stack = collections.defaultdict(
                list)
        ...
        with self._lock:
            ...
            if add_shapes:
                ...
                for function_def in graph.library.function:
                    ...
                    assert len(input_shapes.list.shape) in [
                        0, len(func_graph_inputs)]
                    ...
                    if not input_shapes.list.shape:
                        for input_tensor, arg_def in zip(func_graph_inputs,
                            ...
                            if input_tensor.dtype == dtypes.resource:
                                _copy_handle_data_to_arg_def(
                                    input_tensor, arg_def)
    ...
    def _create_op_helper(self, op, compute_device=True):
        ...
        for key, value in self._attr_scope_map.items():
            try:
                ...
                if value:
                    op._set_attr(
                        key, value)  # pylint: disable=protected-access
        ...
        op._gradient_function = self._gradient_function_map.get(
            op.type)  # pylint: disable=protected-access
        ...
        if self._colocation_stack:
            ...
            for colocation_op in self._colocation_stack.peek_objs():
                try:
                    all_colocation_groups.extend(
                        colocation_op.colocation_groups())
                except AttributeError:
                    pass
    ...
    def _add_new_tf_operations(self, compute_devices=True):
        ...
        new_ops = [
            self._create_op_from_tf_operation(
                c_op, compute_device=compute_devices)
            for c_op in c_api_util.new_tf_operations(self)
        ]

        # pylint: disable=protected-access
        for op in new_ops:
            new_control_inputs = self._control_dependencies_for_inputs(
                op.inputs)
    ...
    def _as_graph_element_locked(self, obj, allow_tensor, allow_operation):
        ...
        if allow_tensor and allow_operation:
            ...
            raise ValueError(
                "allow_tensor and allow_operation can't both be False.")
        ...
        if isinstance(obj, compat.bytes_or_text_types):
            ...
            if obj.graph is not self:
                raise ValueError(
                    "Tensor %s is not an element of this graph." % obj)
            return obj
        elif isinstance(obj, Operation) and allow_operation:
            # Actually obj is just the object it's referring to.
            if obj.graph is not self:
                raise ValueError(
                    "Operation %s is not an element of this graph." % obj)
            return obj
        else:
            # We give up!
    ...
    def name_scope(self, name):
        ...
        * A value of `None` or the empty string will reset the current name scope
        ...
        # Both for name=None and name="" we re-set to empty scope.
        if not name:
            new_stack = ""
            returned_scope = ""
        elif name[-1] == "/":
        ...
        **NOTE** Using a colocation scope resets any existing device constraints.
        ...
        if op is not None:
            ...
            if device_only_candidate is not None:
                self._colocation_stack.push_obj(
                    device_only_candidate, offset=4)
...
# pylint: disable=protected-access
        ...
        def __exit__(self, unused_type, unused_value, unused_traceback):
            ...
            if self._new_stack:
                ...
                self._graph._set_control_flow_context(
                    self._old_control_flow_context)
    ...
    def _control_dependencies_for_inputs(self, input_ops):
        ...
        for controller in self._control_dependencies_stack:
            ...
            if not dominated:
                ...
                ret.extend(
                    c for c in controller.control_inputs if c not in input_ops)
        return ret
        ...
        *N.B.* The control dependencies context applies *only* to ops that
        ...
        for c in control_inputs:
            ...
            if isinstance(c, Tensor):
                ...
                raise TypeError(
                    "Control input must be Operation or Tensor: %s" % c)
            if c not in current:
...
def _get_outer_context_and_inner_device_stack():
    ...
    if not _default_graph_stack.stack:
        ...
        if outer_context is None:
            ...
            outer_context = _default_graph_stack._GetGlobalDefaultGraph(
...
def dismantle_graph(graph):
    ...
    """
    memory.dismantle_ordered_dict(
        graph._functions)  # pylint: disable=protected-access
...
def _op_to_colocate_with(v, graph):
    ...
    if hasattr(v, "handle") and isinstance(v.handle, Tensor):
        def device_only_candidate(): return None
...
def add_exit_callback_to_default_func_graph(fn):
    ...
    default_graph._add_scope_exit_callback(
        fn)  # pylint: disable=protected-access
...
def set_int_list_attr(op, attr_name, ints):
    ...
    op._set_attr(attr_name, attr_value_pb2.AttrValue(
        list=ints_list))  # pylint:disable=protected-access
